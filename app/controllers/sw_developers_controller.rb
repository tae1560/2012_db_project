  ...
  def services
    ...
    @services.each do |service|
      ...
      service.teams.each do |team|
        ...
        # pro_field 별 횟수 받아오기
        pro_fields = service.pro_fields
        number_of_developers_in_service = {}
        service.pro_fields.each do |pro_field|
          if number_of_developers_in_service[pro_field.id]
            number_of_developers_in_service[pro_field.id] += 1
          else
            number_of_developers_in_service[pro_field.id] = 1
          end
        end
        ...
        number_of_developers_in_service.each do |pro_field_id, number_of_developer|
          if !number_of_developers_in_team[pro_field_id] or number_of_developer > number_of_developers_in_team[pro_field_id]
            need_pro_fields.push pro_field_id
          end
        end
        ...
        team_data["need_pro_fields"] = need_pro_fields
        team_data["team"] = team

        if team_reader == @sw_developer
          service_datum["own_team"] = team_data
        elsif need_pro_fields.size == 0
